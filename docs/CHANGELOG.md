...
## [v1.3.4] - 2026-09-08 14:43

### 优化
- 日志搜索左侧筛选：Service / trace_id / request_id / user_id / 接入应用 / 关键词统一为
  「上标题 + 下输入框」样式，单列纵向排列，保持一致对齐
- 日志搜索选择非输入条件（快捷区间、等级、环境、自定义时间）后直接触发搜索
- 导出 CSV / JSONL 按钮成组一行，加导出标识
- Dashboard 左侧筛选（Environment / Service / 接入应用）改为每个条件独立一块、纵向排列

## [v1.3.3] - 2026-09-08 13:49

### 变更
- 日志搜索默认每页 25 条
- 日志搜索页改为视口内不上下滚动：主体应用外壳（顶栏固定、内容区滚动），
  搜索结果表格在容器内独立滚动，页面整体不滚动
