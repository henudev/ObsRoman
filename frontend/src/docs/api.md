...
**异常码**：`1303`（范围超 24 小时）、`1304`（格式不支持）、`1401/1402`。
...
**返回参数**：`{ "status": "UP", "version": "1.3.4", "release_date": "2026-09-08 14:43" }`
...
- **v1.3.4**（2026-09-08）：日志搜索/ Dashboard 左侧筛选样式统一；选择非输入条件即触发搜索；导出按钮成组。
- **v1.3.3**（2026-09-08）：日志搜索默认每页 25 条；页面改为视口内不上下滚动。
- **v1.3.2**（2026-09-08）：API / SDK 文档页新增左侧目录导航（点击跳转 + 当前章节高亮）。
...
- **v1.2.3**（2026-09-01）：清空数据窗口期查询与写入健壮性优化（无接口变更）。
- **v1.2.2**（2026-09-01）：Trace 页返回导航 / 搜索页状态保留 / 数据清空降级（无接口变更）。
- **v1.2.1**（2026-09-01）：Dashboard 卡片与搜索筛选面板改版（无接口变更）。
